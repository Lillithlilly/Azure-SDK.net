// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

namespace Azure.Communication.CallingServer
{
    /// <summary> The base class for all the responses for any Acs Api operation. </summary>
    public partial class OperationResponse
    {
        /// <summary> Initializes a new instance of OperationResponse. </summary>
        /// <param name="status"> The status of the operation. </param>
        internal OperationResponse(CallingOperationStatus status)
        {
            Status = status;
        }

        /// <summary> Initializes a new instance of OperationResponse. </summary>
        /// <param name="operationId"> The operation id. </param>
        /// <param name="status"> The status of the operation. </param>
        /// <param name="operationContext"> The operation context provided by client. </param>
        /// <param name="resultInfo"> The result info for the operation. </param>
        internal OperationResponse(string operationId, CallingOperationStatus status, string operationContext, CallingOperationResultDetails resultInfo)
        {
            OperationId = operationId;
            Status = status;
            OperationContext = operationContext;
            ResultInfo = resultInfo;
        }

        /// <summary> The operation id. </summary>
        public string OperationId { get; }
        /// <summary> The status of the operation. </summary>
        public CallingOperationStatus Status { get; }
        /// <summary> The operation context provided by client. </summary>
        public string OperationContext { get; }
        /// <summary> The result info for the operation. </summary>
        public CallingOperationResultDetails ResultInfo { get; }
    }
}
